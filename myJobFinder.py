...
import requests, os, datetime, webbrowser, openpyxl, pyautogui, time
...
searchResultElt = browser.find_element_by_css_selector('h1.pivot') # this elt is defined after the submit button because it is on the 2nd page.

# check if some jobs have been found and download them.
if searchResultElt.text != "Sorry, we didn't find any jobs matching your criteria":
    while True:
        try: # click the load more elt button to display all jobs.
    ...
    newFolder = 'Jobs_%s_for_%s' %(datetime.date.today().strftime("%a_%b_%d"), postingDate) # E.g: Jobs_Mon_Oct_14_for_Yesterday
    ...
    jobs = True # boolean value that shows if any job has been found for the criteria provided.
else:
    print("Sorry, we didn't find any jobs matching your criteria")
    jobs = False 
...
if jobs:
   ...
   message = "Sorry, no job has been posted for %s since %s in %s." %(jobTitle, postingDate, jobLocation)
   textMe(message)
   
time.sleep(5) # make a 5 seconds pause to allow the user to read messages before closing the program.
